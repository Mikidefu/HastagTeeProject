...
export class CartComponent implements OnInit, OnDestroy, AfterContentChecked {
    ...
    ngOnInit() {
        ...
        this.sub = this.updateTerms.pipe(
            // attende 300ms dopo ogni keystroke prima di considerare i termini
            debounceTime(300),
            // Switcha al nuovo Observable cercato ogni volta che i termini cambiano
            //
            switchMap((productInOrder: ProductInOrder) => this.cartService.update(productInOrder))
        ).subscribe(prod => {
                if (prod) { throw new Error(); }
            },
            _ => console.log('Update Item Fallito'));
    }
    ...
    remove(productInOrder: ProductInOrder) {
        this.cartService.remove(productInOrder).subscribe(
            ...
            _ => console.log('Remove Cart Fallito'));
    }
    ...
    checkout() {
        if (!this.currentUser) {
            ...
            this.cartService.checkout().subscribe(
                ...
                error1 => {
                    console.log('Checkout Carrello Fallito');
                });
